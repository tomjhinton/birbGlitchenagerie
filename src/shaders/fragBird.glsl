...
float fbm ( in vec2 _st, int NUM_OCTAVES) {
    float v = 0.0;
    float a = 0.5;
...
vec3 makeSwirl(in vec2 p0, float swirlIntensity) {
    float a = atan(p0.y, p0.x);
    float len = length(p0);
    float l = log(len) - vTime * .4;
    float zigzag = sin(vTime * .5 + a * 10. + l * 20.);
    float swirl = sin(-vTime * .5 + a * 4. + l * 10. + zigzag);
    float bg = .5 + .5 * sincos(p0 - l * 2.);
    float blur = .2 + max(0., .5 - len);
    return twilight(vTime * .1 + l * .2) * vec3(bg) * .5 + .5 * smoothstep(0., blur, swirl) * .01 * swirlIntensity;
  }
...
void main(){
  ...
  uv = vUv;
  uv = tile(uv, uValueB * 2. + .5);

  uv += cnoise(uv * (uValueC * 5.));
